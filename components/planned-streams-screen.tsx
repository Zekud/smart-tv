...
interface PlannedStreamsScreenProps {
  onChannelSelect: (videoId?: string) => void;
  onWatchLive: (videoId: string) => void;
  onWatchPlanned: () => void;
}

export default function PlannedStreamsScreen({
  onChannelSelect,
  onWatchLive,
  onWatchPlanned,
}: PlannedStreamsScreenProps) {
      ...
      <div className="flex">
        ...
        <div className="flex-1">
          <div className="bg-gray-800 bg-opacity-50 px-4 py-2 rounded-md inline-block mb-6">
            <h2 className="text-xl font-bold">Planned Streams</h2>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 mb-8">
            {mainCat.map((category, index) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                onClick={onWatchPlanned}
              >
                <div className="relative w-full">
                  <Image
                    src={category.src}
                    alt={`Planned stream ${index + 1}`}
                    width={520}
                    height={300}
                    className="w-full h-full object-cover"
                  />
                  {/* Gradient overlay from left (lighter) to right (darker) */}
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent to-black/85"></div>
                  ...
                  {/* Time indicator */}
                  <div className="absolute bottom-3 right-3 bg-gray-700 bg-opacity-80 text-white text-xs px-3 py-1.5 rounded-md flex items-center gap-1.5">
          ...
          <div className="mb-8">
            <div className="bg-red-600 text-white px-4 py-2 rounded-md inline-block mb-6">
              ON Air
            </div>
